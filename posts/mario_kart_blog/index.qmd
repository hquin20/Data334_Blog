...
what data set are you working with? - mario kart

what variables are you most interested in? - track, shortcut, time

How many observations are in your data set? - 2334 overall

what is your question of interest? - Which track is the fastest?
...
*For how many tracks have shortcuts been discovered?* - 12 out of 16 tracks have shortcuts
...
(If possible, give a link to the data source). - Mario Kart World Records

what other pieces of context are important to explain to the reader? - looking at single laps
...
record_sum_nshortcut |>
  ...
  geom_segment(aes(xend = track, yend = 0), color = "grey65", linewidth = 1.5) +
  geom_point(size=3.5, color="red", fill=alpha("orange"), shape=21, stroke=1.5) +
  coord_flip() +
  theme_minimal() +
  ylab("Average Time (s)") +
  xlab("Track") +
  labs(title = "Average Time to Complete One Lap (Non-Shortcut)",
  caption = "Data Source: Mario Kart World Records")
```

```{r CHUNK 4, echo = F, eval = F, warning = F}
record_sum_shortcut |>
  mutate(track = fct_reorder(track, avg_time)) |>
  ggplot(aes(x = track, y = avg_time)) +
  geom_segment(aes(xend = track, yend = 0), color = "grey65", linewidth = 1.5) +
  ...
  ylab("Average Time (s)") +
  xlab("Track") +
  labs(title = "Average Time to Complete One Lap (Shortcut)",
  caption = "Data Source: Mario Kart World Records")
```
